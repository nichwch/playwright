...
<div class="relative h-96 overflow-y-auto overscroll-none border border-black">
	<div class="absolute top-0 h-full w-full">
		<!-- THE UNDERLAY -->
		<div class="absolute top-0 h-full w-full whitespace-pre-line p-3 leading-6">
		...
		<div class="absolute top-0 h-full w-full whitespace-pre-line p-3 leading-6">
			...
			{/if} -->
			<span class="relative inline text-transparent">
				{textBeforeCaret}
			</span>

			<span class="inline-block h-6 w-1" id="caret" bind:this={caretRef} />
			<!-- SLASH MENU -->
			<div
				style:position="absolute"
				class:hidden={!showingSlashMenu}
				class="top-10 z-40 h-24 w-72 overflow-y-scroll border border-black bg-white"
			...
			>
				{#each shownMenuOptions as option, index}
					<button
						...
						class="block w-full px-2 text-left hover:bg-red-100"
						on:click={() => {
		...
		/>
		<!-- THE OVERLAY -->
		<div class="h-fullp-3 absolute top-0 w-full whitespace-pre-wrap leading-6">
			{#each formatted_paragraphs as paragraph, paragraph_index}
				{#each paragraph as clause, clause_index}
					...
					{#if match !== undefined && location !== undefined}
						...
						{:else}
							<span
								class="z-20 bg-red-200 leading-6 outline outline-black hover:bg-red-300"
								style:position="absolute"
